...
idxs = torch.Tensor(range(len(videoTensor))).type(torch.long).view(1,-1).unfold(1,size=nbr_frame,step=1).squeeze(0)#len(tensor)为维度，
...
n_inputs = len(frames)
width = n_outputs + 1#等于arg.factor
...
for i in tqdm.tqdm(range(len(idxs))):#tqdm为进度条
    idxSet = idxs[i]
    ...
    outputs.extend(outputFrame)#拆开后加入
