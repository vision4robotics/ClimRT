...
class TrkDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        previous_item=cv2.imread(previous[-1][0])
    
        previous_searchbox = self._get_bbox(previous_item, previous[-1][1])
    
        previous_simg, _ = self.search_aug(previous_item,
                                            previous_searchbox,
                                            cfg.TRAIN.SEARCH_SIZE,
                                            rand,
                                            gray=gray)
        #print(previous_simg.shape())
        ...
        #current_templateimage = torch.clamp(current_templateimage,0,1)
        current_searchimg = current_searchimg.transpose((2, 0, 1)).astype(np.float32)
        #current_searchimg = torch.clamp(current_searchimg,0,1)
        previous_simg = previous_simg.transpose((2, 0, 1)).astype(np.float32)
        #previous_simg = torch.clamp(previous_simg,0,1)#此处仍为numpy
    

        return {
                'pre_search': previous_simg,
