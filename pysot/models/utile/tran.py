...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor,srcc: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        src = src.permute(1,2,0).view(b,c,int(s**0.5),int(s**0.5))
        srcc = srcc.permute(1,2,0).view(b,c,int(s**0.5),int(s**0.5))
        #src.reshape(b,c,int(s**0.5),int(s**0.5))
        #srcc.reshape(b,c,int(s**0.5),int(s**0.5))
        #src.contiguous().view(b,c,9,9)
        #srcc.contiguous().view(b,c,9,9)
        srcm = src
        g1 = self.avg_pool(src)
        g2 = self.max_pool(srcc)
        w = g1 + g2
        w = self.conv1(w)
        w = torch.sigmoid(w)
        srcm = w * srcm
        srcm = srcm.view(b,c,-1).permute(2, 0, 1)
        src = src.view(b,c,-1).permute(2, 0, 1)
        srcc = srcc.view(b,c,-1).permute(2, 0, 1)
        src = self.norm1(src + srcm)

        src2 = self.self_attn(src, srcc, srcc, attn_mask=src_mask,
                               key_padding_mask=src_key_padding_mask)[0]
        src2 = self.dropout1(src2)
        src = src.view(b,c,int(s**0.5),int(s**0.5))
        src2 = src2.view(b,c,int(s**0.5),int(s**0.5))
        src = torch.cat((src, src2), 1)
        src = self.conv2(src)
        src = src.view(b,c,-1).permute(2, 0, 1)
