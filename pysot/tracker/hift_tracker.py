...
class ClimRTTracker(SiameseTracker):
    ...
    def track(self, img):
        ...
        #print(x_crop.size())
        #yimage = yimage.cpu().data
        #styimage = make_image(yimage)
        crop_image = x_crop.squeeze(0).cpu().data
        stcrop_image = make_image(crop_image)
        iframe = outputs['iframe']
        iframe = iframe.squeeze(0).cpu().data 
        #print(iframe.size())
        stiframe = make_image(iframe)

        ls = outputs['ls']
        #print(ls.size())
        ls = ls.squeeze(0).cpu().data 
        stls = make_image(ls)

        ns = outputs['ns']
        #print(ls.size())
        ns = ns.squeeze(0).cpu().data 
        stns = make_image_01(ns)
